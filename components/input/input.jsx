...
export class Input extends React.Component {
    ...
    onChange (e) {
        const{value, hide, onInputChange, name, patern} = this.props;
        let regex = new RegExp(patern);
        onInputChange(name, e.target.value, regex.test(e.target.value));
       
        if(!value.length && (e.target.name ==='inputPassword')){
            this.btnHide = true;
            hide(this.btnHide);
        }
    }
    ...
    componentDidUpdate(){
        const{value, onInputChange, name, patern} = this.props;
        if(!patern && value){
            onInputChange(name, '', false);
        }
    }
    ...
    render(){
        ...
        return(
            <div className='input-template'>
                ...
                <div className='input-template__icon' style={{display: name === 'inputEmail' || name === 'inputName'  || name === 'inputUsername' ? 'flex': 'none'}}>
                ...
                <div className='input-template__icon' style={{background: !patern ? 'rgba(0, 0, 0, 0.02)': 'white'}} onClick={this.onClick} style={{display: name === 'inputPassword' || name === 'inputPassword1' ? 'flex': 'none'}}>
                ...
                <div className='input-template__icon' style={{display: name === 'inputEmail2' ? 'flex': 'none'}}>
